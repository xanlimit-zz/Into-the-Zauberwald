package{
	...
	public class Main4 extends MovieClip {
		...
		public function Main4() {
			...
			player.x = 100;
			player.y = Main.getStage().stageHeight - 600;
		...
		function onEnterFrameHandler(e: Event) {
			...
			if (collisions.touch_mc.hitTestObject(player)) {
				if(Main.coinCount == 6){
					successGame();
				}
			...
			coins_txt.coins_txt.text = Main.coinCount + "/6"  ;
		}
		...
		function successGame() {
			...
			var afterWaiting: Function = function (event: TimerEvent): void {
				...
				var afterWaiting2: Function = function (event: TimerEvent): void {
					...
					var e2:fourthEnd = new fourthEnd;
					...
					var afterWaiting3: Function = function (event: TimerEvent): void {
						...
						var nextStage:Function = function(e:KeyboardEvent){
							...
							sndBGMusicChannel.stop();
							Main.coinCount = 0;
							var ed: ending_screen = new ending_screen;
							Main.getStage().addChild(ed);
							ed.x += (Main.getStage().width/2);
							ed.y += (Main.getStage().height/2)-23;
